...
chunk _crit txts
  = case traverse (parseString javaFile . T.unpack) txts of
      (Right vs) -> let findDependents k
                          = Map.findWithDefault (error "Map missing key") k dependents
...
class Silence a where
  silence :: Set.Set Text -> a -> a -- ^ remove everything not in the set, leave only the contracts for what is in the set

instance Splittable ClassItem where
  split _ o@(Method _ hdr (Sequential _)) -- TODO: find a way to silence Mathematical too. Requires a VerCors adjustment
    = Set.singleton (extractName hdr, o)
  split _ _ = Set.empty
...
instance (Splittable a, Silence a) => Splittable [a] where
  split s (x:xs)
   = Set.union (Set.map (\(idT,v) -> (idT,v:map (silence (s idT)) xs)) (split s x))
               (Set.map (\(idT,v) -> (idT,silence (s idT) x:v)) (split s xs))
  split _ [] = Set.empty

-- instance Silence a => Silence [a] where
...
class Dependencies a where
  getDependencies :: a -> Map.Map Text (Set.Set Text)
class Ids a where
  getIds :: a -> Set.Set Text
...
instance Normalise [BalancedPart] where
  normalise (x:rs)
   = (case x of
       (NoBrackets (TextNoComment txt)) | T.null txt -> id
       o -> (:) o) (normalise rs)
  normalise [] = []
...
instance WhiteSpaced Text where
  removeWS
   = T.intercalate " "
     . L.filter (not . T.null)
     . concat . map (T.splitOn "\r")
     . concat . map (T.splitOn "\n")
     . T.splitOn " "
